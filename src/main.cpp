...
template<typename T>
constexpr size_t mem_size(const std::vector<T> &v) {
...
{    
    ...
    auto device_id = find_gpu_or_cpu();
    ...
    cl_int errcode_ret{};
    Resource<cl_context, clReleaseContext> ctx(clCreateContext(nullptr, 1, &device_id, nullptr, nullptr, &errcode_ret));
    ...
    Resource<cl_command_queue, clReleaseCommandQueue> queue = clCreateCommandQueue(ctx, device_id, 0, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    using mem_res_t = Resource<cl_mem, clReleaseMemObject>;
    mem_res_t as_gpu = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, mem_size(as), as.data(), &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    mem_res_t bs_gpu = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, mem_size(bs), bs.data(), &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    mem_res_t cs_gpu = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, mem_size(cs), cs.data(), &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    auto kernel_cstr = kernel_sources.c_str();
    Resource<cl_program, clReleaseProgram> prog = clCreateProgramWithSource(ctx, 1, &kernel_cstr, nullptr, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    Resource<cl_kernel, clReleaseKernel> kernel = clCreateKernel(prog, "aplusb", &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);

    // TODO 10 Выставите все аргументы в кернеле через clSetKernelArg (as_gpu, bs_gpu, cs_gpu и число значений, убедитесь что тип количества элементов такой же в кернеле)
    {
         unsigned int i = 0;
         OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(as_gpu.data), &as_gpu.data));
         OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(bs_gpu.data), &bs_gpu.data));
         OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cs_gpu.data), &cs_gpu.data));
    ...
    constexpr double gb = 1024. * 1024. * 1024.;
    {
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << 3 * mem_size(as) / gb / t.lapAvg() << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            clEnqueueReadBuffer(queue, cs_gpu, true, 0, mem_size(cs), cs.data(), 0, nullptr, nullptr);
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << mem_size(cs) / gb / t.lapAvg() << " GB/s" << std::endl;
    }
